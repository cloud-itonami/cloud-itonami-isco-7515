(ns foodtaste.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [foodtaste.actor :as actor]
...
(deftest commits-a-registered-sample-log
  (let [st (fresh-store)
        ...
        request {:taster-id "taster-1" :op :log-sample-record :stake :low
                  :facility-id "FAC-1" :item "batch 44 lot A intake"}
        result (actor/run-request! graph request {} "thread-1")]
    (is (= :done (:status result)))
    (is (some? (get-in result [:state :record])))
    (is (= 1 (count (store/records-of st "taster-1"))))))
...
(deftest holds-an-unregistered-facility-proposal
  (let [st (fresh-store)
    ...
    (is (empty? (store/records-of st "taster-1")))))

(deftest interrupts-then-approves-anomaly-concern-on-human-approval
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:taster-id "taster-1" :op :flag-anomaly-concern :stake :low
                  :facility-id "FAC-1" :anomaly-type :off-odor-reported}
...
(deftest holds-a-scope-excluded-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would assign a quality grade, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:taster-id "taster-1" :op :assign-quality-grade :stake :low
...
(deftest holds-a-pass-fail-determination-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would determine a pass/fail outcome, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:taster-id "taster-1" :op :determine-pass-fail-outcome :stake :low
...
(deftest holds-a-food-safety-clearance-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize a food-safety clearance, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:taster-id "taster-1" :op :finalize-food-safety-clearance :stake :low
                    :facility-id "FAC-1" :item "food-safety clearance decision"}
          result (actor/run-request! graph request {} "thread-6")]
      (is (= :done (:status result)))
      (is (= :hold (:disposition (:state result))))
      (is (empty? (store/records-of st "taster-1"))))))

(deftest holds-a-perform-sensory-evaluation-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would perform the sensory evaluation itself — no robot can taste"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:taster-id "taster-1" :op :perform-sensory-evaluation :stake :low
                    :facility-id "FAC-1" :item "sensory evaluation"}
